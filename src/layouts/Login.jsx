...
const Login = () => {
  ...
  const { signwithg, manualsignin, forgetpemail } = use(Contextapi)
  ...
  const handleforget = () => {
    const gotmail = emailref.current.value
    forgetpemail(gotmail)
  }
  const [show, setShow] = useState(false);
  return (
    <div className="my-10">
      <p className="text-4xl my-8 text-center font-black">Login now!</p>
        ...
        <div className="card-body w-90">
          <fieldset className="fieldset">
            <form onSubmit={handlemanuallogin} action="">
              ...
              <input required ref={emailref} type="email" name='email' className="input placeholder-gray-400" placeholder="Email" />
              ...
              <div onClick={handleforget}><Link to={'/forget'} className="text-blue-600">Forgot password?</Link></div>
              <p className="text-[15px] text-red-500">{msg}</p>
              <button className="btn bg-black text-white w-full mt-4">Login</button>
            </form>
            ...
            <div className="text-center my-1">
              <p className="font-semibold text-[15px]">New here? <Link className="text-blue-500" to={'/signup'}>Sign up</Link></p>
            </div>
          </fieldset>
        </div>
      </div>
